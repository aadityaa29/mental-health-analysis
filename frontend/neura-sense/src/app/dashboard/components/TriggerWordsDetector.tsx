 ...
 * Accepts a list of insights: { text, timestamp }
 * Computes trigger words + examples
 */
...
export default function TriggerWordsDetector({ insights, topN = 12 }: Props) {
  // ✅ Use fallback if insights is undefined OR empty
  const items =
  ...
  const topWords = useMemo(() => {
    ...
    for (const s of items) {
      ...
      for (const w of uniq) {
        ...
        if (
          ["the", "and", "for", "you", "your", "that", "this", "with", "have", "just", "not"]
            .includes(w)
        )
          continue;
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex gap-2 flex-wrap">
          {topWords.length === 0 && (
            <div className="text-gray-500">No trigger words found.</div>
          )}
        ...
        {selected && (
          <div className="mt-4">
            ...
            <div className="text-xs text-gray-500 mt-2">
              Tap a word to view example snippets where it appears.
            </div>
          </div>
        )}
      </CardContent>
...
function fallbackInsights(): Insight[] {
  ...
  return [
    {
      text: "I am so anxious about the interview",
      timestamp: new Date(now - 86400000 * 2).toISOString(),
    },
    {
      text: "Nothing seems to be working, feeling depressed",
      timestamp: new Date(now - 86400000).toISOString(),
    },
    {
      text: "Had a panic attack after seeing the message",
      timestamp: new Date(now - 3600000).toISOString(),
    },
    {
      text: "I enjoy coding and exploring new things",
